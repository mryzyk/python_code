...
def load_map(map_filename):
    ...
    with open(map_filename, 'r') as file:
        for line in file:
            ...
            src, dest, total_distance, outdoor_distance = cols
            n1 = Node(str(src))
            n2 = Node(str(dest))
            ...
            if not g.has_node(n2):
                g.add_node(n2)
            e = WeightedEdge(n1,n2, int(total_distance), int(outdoor_distance))
            g.add_edge(e)
    file.close()
    return g
    ...
    for edge in digraph.get_edges_for_node(start_node):
        if edge.get_destination() not in path[0]:
            ...
            if new_outdoor_dist <= max_dist_outdoors:
                    ...
                    new_path = get_best_path(digraph, new_start.get_name(), end, [path[0], new_total_dist, new_outdoor_dist]
                                             ,  max_dist_outdoors, best_dist, best_path)
                    # print(new_path)
                    if new_path is not None and (best_path is None or new_path[1] < best_dist):
                        best_path = new_path
...
def directed_dfs(digraph, start, end, max_total_dist, max_dist_outdoors):
    ...
    print(result)
    if result is not None and result[1] < max_total_dist:
        res = []
...
if __name__ == "__main__":
    ...
    # # # print(str(g.get_node('32')))
    # # # print(Node(32).get_name())
    # print(g)
    # # print(get_best_path(g, '1', '32', [[], 0, 0], 20, 0, None))
    # print(directed_dfs(g, '2', '9', 999999, 0))
    ...
    # print(g)
    # # print(d1)
    # print(get_best_path(g, 'a', 'e', [[], 0, 0], 20, 30, None))
